	...
	, m_pTitleBackUI	( nullptr )
{
	...
	m_pTitleBackUI		= new CTitleBackUI	();
	for (int i = 0; i < Controller_Max; i++)
	{
		...
		m_pTitleBackUI->SetXInput(m_pController	[i], i);
	}
...
void CGameScenePlayerSetup::Update()
{
	...
	if (m_pGameRdyUI->GetHandclapflag(0) == false)
	{
		m_pTitleBackUI->Update();
	}
}
...
void CGameScenePlayerSetup::Draw()
{
	...
	m_pTitleBackUI->Draw();		//タイトルに戻るときに描画されるUI.
