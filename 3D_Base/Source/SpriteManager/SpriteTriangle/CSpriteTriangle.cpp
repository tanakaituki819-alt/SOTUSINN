...
#include "System/DirectX/CDirectX11.h"

//シェーダファイル名（ディレクトリも含む）.
const TCHAR SHADER_NAME[] = _T("Data\\Shader\\Sprite3D.hlsl");

//コンストラクタ.
CSpriteTriangle::CSpriteTriangle()
	: m_pDx11(nullptr)
	, m_pDevice11(nullptr)
...
//===========================================================
HRESULT CSpriteTriangle::CreateShader()
{
...
//モデル作成.
HRESULT CSpriteTriangle::CreateModel()
{
	...
	D3DXVECTOR3 UE = { 0,h,0 };
	D3DXVECTOR3 HIDARITEMAE={ -w,-h,-d };
	D3DXVECTOR3 MIGITEMAE = { w,-h,-d };
	D3DXVECTOR3 HIDARIOKU = { w,-h,d };
	D3DXVECTOR3 MIGIOKU = { -w,-h,d };


	UE = MakeRotation(UE, 90 * M_PI / 180, 0);
	HIDARITEMAE =MakeRotation(HIDARITEMAE, 90 * M_PI / 180, 0);
	MIGITEMAE  =MakeRotation(MIGITEMAE, 90 * M_PI / 180, 0);
	HIDARIOKU  =MakeRotation(HIDARIOKU, 90 * M_PI / 180, 0);
	MIGIOKU = MakeRotation(MIGIOKU, 90 * M_PI / 180, 0);
	...
	{
		// --- 手前面 ---
		{  UE, D3DXVECTOR2(0.5f,     0.0) }, { HIDARITEMAE, D3DXVECTOR2(0,     1) }, { MIGITEMAE, D3DXVECTOR2(1.0, 1.0f) },
		// --- 奥面 ---
				{ UE, D3DXVECTOR2(0.5f,     0.0) }, { MIGIOKU, D3DXVECTOR2(0,     1) }, { MIGITEMAE, D3DXVECTOR2(1.0, 1.0f) },
		// --- 右面 ---
				{  UE, D3DXVECTOR2(0.5f,     0.0) }, { MIGIOKU, D3DXVECTOR2(0,     1) }, {  HIDARITEMAE, D3DXVECTOR2(1.0, 1.0f) },
		
			// --- 左面 ---
				{ UE, D3DXVECTOR2(0.5f,     0.0) }, {HIDARIOKU , D3DXVECTOR2(0,     1) }, {  MIGITEMAE, D3DXVECTOR2(1.0, 1.0f) },
			// --- 下面 ---
			{ D3DXVECTOR3{-w,-h,d}, D3DXVECTOR2(0,0)}, {HIDARIOKU, D3DXVECTOR2(1,     0)},{ HIDARITEMAE, D3DXVECTOR2(0, 1.0f)},
			{ HIDARITEMAE, D3DXVECTOR2(0,1)}, {MIGIOKU, D3DXVECTOR2(1,     1)}, {HIDARIOKU, D3DXVECTOR2(1.0, 0.0f)},
...
//テクスチャ作成.
HRESULT CSpriteTriangle::CreateTexture(LPCTSTR lpFileName)
{
	//テクスチャ作成.
...
//サンプラ作成.
HRESULT CSpriteTriangle::CreateSampler()
{
	//テクスチャ用のサンプラ構造体.
	...
	D3DXMATRIX& mView, D3DXMATRIX& mProj)
{
	...
	//プリミティブをレンダリング.
	m_pContext11->Draw(4*3, 0);//板ポリ（頂点4つ分）.
...
void CSpriteTriangle::RenderY(D3DXMATRIX& mView, D3DXMATRIX& mProj)
{
	//ワールド行列.
	D3DXMATRIX	mWorld;
	D3DXMATRIX	mTrans, mRot, mScale;
	...
	//アルファブレンド無効にする.
	m_pDx11->SetAlphaBlend(false);
	D3DXVECTOR3 MakeRotation(D3DXVECTOR3 pos, float Delection_X, float Delection_Y);
}

D3DXVECTOR3 CSpriteTriangle::MakeRotation(D3DXVECTOR3 pos, float Delection_X, float Delection_Y)
{
		D3DXVECTOR3 point;
		D3DXVECTOR3 point2;
		double XDer = Delection_X;
		double YDer = Delection_Y;
		//x軸回転
